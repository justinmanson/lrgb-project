...
def make_k_hop_edges(dataset, K, format, name, split):
  ...
  file_exists = [exists(join(filedir, "%s-%s-%s%s_k=%02d.pt" % (split, format, name, extra, k))) for k in range(1,K+1)] # list of K bools
  if not all(file_exists): # checks all files are there
    ...
    print('Edge index file(s) not found for %s-%s-%s%s_k=%02d (or lower); making file(s) now...' % (split, format, name, extra, last_nonexistent_file))
    compute_k_hop_edges(dataset, K, filedir, format, name, split, extra) # if they don't, make them

  # load files
  all_graphs = []
  print('Loading k-hop data files...')
  for k in tqdm(range(1,K+1)):
    filepath = join(filedir, "%s-%s-%s%s_k=%02d.pt" % (split, format, name, extra, k))
    try:
      ...
      for j in range(K, k, -1):
        try: 
          torch.load(join(filedir, "%s-%s-%s_k=%02d.pt" % (split, format, name, k)))
        except:
          files_to_remake.append(j)
      print('Issue with following files, deleting and remaking them...\nk = ', files_to_remake)
      for j in files_to_remake:
        filepath = join(filedir, "%s-%s-%s_k=%02d.pt" % (split, format, name, j))
        os.remove(filepath)
      compute_k_hop_edges(dataset, max(files_to_remake), filedir, format, name, split)
      filepath = join(filedir, "%s-%s-%s_k=%02d.pt" % (split, format, name, k))
      all_graphs.append(torch.load(filepath)) # [K,N,2,d]
...
def compute_k_hop_edges(dataset, K, filedir, format, name, split, extra):
